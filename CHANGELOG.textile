...
* dashed commandline params are accepted but cause a warning -- they do *not* serve as deep keys. By default Configliere happily accepts them, but you can instead make a middleware that would either restore the old --foo-bar is equivalent to --foo.bar behavior, or would silently convert --foo-bar to --foo_bar.
