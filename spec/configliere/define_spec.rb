...
describe "Configliere::Define" do
  ...
  describe 'type coercion' do
    ...
    it 'converts :now to the current moment' do
      ...
      @config[:param] = 'now' ; @config.resolve! ; @config[:param].should be_within(4).of(DateTime.now)
      @config[:param] = :now  ; @config.resolve! ; @config[:param].should be_within(4).of(DateTime.now)
      @config.define :param, :type => Date
      @config[:param] = :now  ; @config.resolve! ; @config[:param].should be_within(4).of(Date.today)
      @config[:param] = 'now' ; @config.resolve! ; @config[:param].should be_within(4).of(Date.today)
    end
  end
