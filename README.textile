...
Configliere is a no-fuss library for flexibly managing your configuration settings.

A config file, in ~/.configliere/simple_script.yaml
<pre>    :cat:     hat
    :spider:  tuffet
    :sprats:
      :jack:  lean</pre>
...
Output:
<pre>    ./simple_script.rb --sprats.wife=fat --spider=drainspout
    {:cat=>"hat", :spider=>"drainspout", :sprats=>{:jack=>"lean", :wife=>"fat"}}</pre>
