...
* *Know how to keep a frickin secret*. Rather than storing passwords and API keys in plain sight, configliere has a protection racket that can obscure values when stored to disk.
...
* *:type*: converts params to a desired form. It understands Date, Time, Integer, :boolean and Symbol. :blank => nil and :blank => false. Make sure to call Config.resolve! in your script.
* *:description* documents a param.
* *:required* marks params required. Any required parameter found to be nil raises an error (listing all missing params). (Make sure to call Config.resolve! in your script.)
